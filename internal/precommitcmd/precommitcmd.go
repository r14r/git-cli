...
func Run(args []string, out, errOut io.Writer) int {
	...
	case "run":
		...
		if err := runStaged(root, out, errOut); err != nil {
			fmt.Fprintln(errOut, err)
			return 1
		}
		return 0
	...
	if !*setup {
		fmt.Fprintln(errOut, "precommit requires --setup, or use run|status|list|uninstall")
		return 2
	}
	if (*preset == "" && !*scan) || (*preset != "" && *scan) {
		fmt.Fprintln(errOut, "use exactly one of --for <preset> or --scan")
		return 2
	}
	...
	if err := precommit.WriteConfig(root, selected); err != nil {
		fmt.Fprintln(errOut, err)
		return 2
	}
...
func uninstall(out, errOut io.Writer) int {
	...
	if !strings.Contains(string(b), managedMarker) {
		fmt.Fprintln(errOut, "pre-commit hook is not managed by git-cli precommit")
		return 2
	}
	securityOnly := "#!/usr/bin/env bash\nset -e\n\nexec git-cli security check-staged\n"
	if err := os.WriteFile(path, []byte(securityOnly), 0o755); err != nil {
		fmt.Fprintln(errOut, err)
		return 2
	}
	_ = os.Remove(filepath.Join(root, precommit.ConfigFilename))
...
func status(out, errOut io.Writer) int {
	...
	if err != nil {
		fmt.Fprintf(out, "Preset: not configured (%s)\n", precommit.ConfigFilename)
		return 0
	}
...
func runStaged(root string, out, errOut io.Writer) error {
	...
	if len(files) == 0 {
		fmt.Fprintln(out, "No staged files.")
		return nil
	}
	...
	case "django":
		if err := runPython(root, materialized, out, errOut); err != nil {
			return err
		}
...
func runPython(root string, files map[string]string, out, errOut io.Writer) error {
	...
	for original, staged := range files {
		if strings.EqualFold(filepath.Ext(original), ".py") {
			py = append(py, staged)
		}
	}
	if len(py) == 0 {
	...
	if commandExists("ruff") {
		return run(root, out, errOut, "ruff", append([]string{"check"}, py...)...)
	}
...
func runPHP(root string, files map[string]string, out, errOut io.Writer) error {
	...
	found := false
	for original, staged := range files {
		if !strings.EqualFold(filepath.Ext(original), ".php") {
			continue
		}
		found = true
		if err := run(root, out, errOut, "php", "-l", staged); err != nil {
			return err
		}
	}
	if !found {
